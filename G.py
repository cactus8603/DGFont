...
class MLP(nn.Module):
    def __init__(self, self, nf_in, nf_out, nf_mlp, num_blocks, norm, act, use_sn=False):
        super(MLP, self).__init__()
        self.model = nn.ModuleList()
        nf = nf_mlp
        self.model.append(LinearBlock(nf_in, nf, norm=norm, act=act, use_sn=use_sn))
        for _ in range(num_blocks - 2):
            self.model.append(Linear(nf, nf, norm=norm, act=act, use_sn=use_sn))
    ...
    def init_fun(m):
        ...
        if(classname.find('Conv') == 0 or classname.find('Linear') == 0) and hasattr(m, 'weight'):
            if init_type == 'gaussian':
                ...
                init.kaining_normal_(m.weight.data, a=0, mode='fan_in')
            elif init_type == 'orthogonal':
                init.orthogonal_(m.weight.data, gain=math.sqrt(2))
            elif init_tpye == 'default':
                pass
            else:
...
def assign_adain_params(adain_params, model):
    # assign the adain params to the AdaIN layers in model
