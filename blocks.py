...
class ResBlocks(nn.Module):
    def __init__(self, num_blocks, dim, norm, act, pad_type, use_sn=False):
        ...
        for i in range(num_blocks):
            self.model.append(ResBlocks(dim, norm=norm, act=act, pad_type=pad_type, use_sn=use_sn))
...
class Conv2dBlock(nn.Module):
    def __init__(self, in_dim, out_dim, ks, st, padding=0,
        ...
        self.use)bias = use_bias

        # init padding
        if pad_type == 'reflect': # reflect + padding
            self.pad = nn.ReflectionPad2d(padding)
        elif pad_type == 'replicate': # replicate the content
            self.pad = nn.ReplicationPad2d(padding)
        elif pad_type == 'zero':
            self.pad = nn.ZeroPad2d(padding)
        else:
            assert 0, "Unsupport padding: {}".format(padding)
...
class FRN(nn.Module):
    def __init__(self, num_features, eps=1e-6):
        ...
        self.beata = nn.Parameter(torch.zeros(1, num_features, 1, 1))
        self.eps = eps
...
class AdaIN2d(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=False, , track_running_stats=True):
        ...
        if self.track_running_stats:
            self.regitset_buffer('running_mean', torch.zeros(num_features))
